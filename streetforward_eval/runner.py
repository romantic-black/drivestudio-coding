    ...
    def _record_iteration(
        ...
        *,
        ...
        for r in metric_rows:
            key = (int(r["eval_frame_id"]), int(r["cam_id"]))
            psnr_by_view[key] = float(r["psnr_non_sky"])
    ...
    def _build_target_frame_order(
        ...
        *,
        ...
        if policy == "all_observed":
            others: List[int] = [
                int(input_frame_ids[b])
                for b in observed_block_order
                if int(b) != int(cur)
            ]
        ...
        if policy == "visited_episode_frames":
            ...
            for b in next_blocks:
                if len(selected_blocks) >= int(limit - 1):
                    break
                selected_blocks.append(int(b))
            out = [int(source_frame)] + [int(input_frame_ids[b]) for b in selected_blocks]
            return out
