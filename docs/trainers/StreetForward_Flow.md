...
**Stage 4.5（No Sky Node）**：`MinimalStreetForwardStage4_5` 基于 4.2，移除 `NodeStateSky` 与 sky 渲染/合成路径；source 2D 改为 scene-only **mask-aware fused multi-camera** 回投（`src_backproject_pass_count=1`，`source_pair_valid_mask` 下沉到 gsplat fused forward/backward）。训练监督上，`RGB/L1 + SSIM` 仅在 `non-sky` 区域（`valid_loss_mask * (1 - sky_mask)`）计算，mask/occupancy 仍对齐 `1 - sky_mask`。配置示例 `configs/minimal_streetforward_stage4_5_multi_scene_v7.yaml`，入口 `tools/train_minimal_streetforward_stage4_5_multi_scene_v7.py`。  
**Stage 4.6（Routed Rigid, No Rigid Heads）**：`MinimalStreetForwardStage4_6` 在 4.5 语义上移除 rigid 专属 decoder/head，保留 rigid local dynamic node，并按 source-frame world 位置将 rigid 路由到 `bg/distant` 共享 heads。配置示例 `configs/minimal_streetforward_stage4_6_multi_scene_v7.yaml`，入口 `tools/train_minimal_streetforward_stage4_6_multi_scene_v7.py`。  
