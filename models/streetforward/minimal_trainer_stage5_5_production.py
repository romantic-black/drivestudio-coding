...
class MinimalStreetForwardStage5_5_Production(
    MinimalStreetForwardStage5_3_Production,
    MinimalStreetForwardStage5_5,
):
    def __init__(self, config, device: torch.device, **kwargs):
        self._validate_production_config(config)
        MinimalStreetForwardStage5_5.__init__(self, config=config, device=device, **kwargs)
    ...
    def _validate_production_config(self, config) -> None:
        ...
        if stage != "5_5":
            raise ValueError("Stage5_5_Production requires model.stage='5_5'.")
        backprojector_version = str(self._require_key(model_cfg, "backprojector_version", "model")).strip().lower()
        if backprojector_version != "v4":
            raise ValueError("Stage5_5_Production requires model.backprojector_version='v4'.")
        if bool(model_cfg.get("use_fused_cuda_backproject_v4", False)) is not True:
            raise ValueError("Stage5_5_Production requires model.use_fused_cuda_backproject_v4=true.")
        ...
        if bool(self._require_key(obs_cfg, "enable", "current_observation")) is not True:
            raise ValueError("Stage5_5_Production requires current_observation.enable=true.")


__all__ = ["MinimalStreetForwardStage5_5_Production"]
