...
class MinimalStreetForwardStage5_2(MinimalStreetForwardStage4_6):
    ...
    def _validate_stage5_2_config(self, config) -> None:
        ...
        if record_views != "source_image_refs":
            raise ValueError("Stage5_2 requires history_memory.record_views=source_image_refs.")
        ...
        for legacy_key in legacy_flat_history_keys:
            if hasattr(history_cfg, "get") and history_cfg.get(legacy_key, None) is not None:
                raise ValueError(
                    "Stage5_2 no longer supports flat history_memory keys. "
        ...
        if feature_extractor_type != "dinov2_unet_fusion":
            raise ValueError("Stage5_2 requires model.feature_extractor.type='dinov2_unet_fusion'.")
    ...
    def _init_stage5_2_modules(self, config) -> None:
        ...
        if feature_extractor_type != "dinov2_unet_fusion":
            raise ValueError(
                f"Stage5_2 unsupported model.feature_extractor.type={feature_extractor_type!r}. "
        ...
        if fusion_out_channels != feat_2d_channels_model:
            raise ValueError(
                "Stage5_2 fusion out_channels must match model.feat_2d_channels "
        ...
        *,
        ...
        if num_distant_total > 0:
            if acc_w_distant is None:
                raise RuntimeError("Stage5_2 support accumulation requires acc_w_distant for active distant branch.")
        ...
        if num_rigid_total > 0:
            ...
            if int(route.S.numel()) > 0:
                if acc_w_rigid_S is None:
                    raise RuntimeError("Stage5_2 support accumulation requires acc_w_rigid_S for active rigid branch.")
                if int(acc_w_rigid_S.shape[0]) != int(route.S.shape[0]):
                    raise RuntimeError("Stage5_2 support accumulation mismatch: len(acc_w_rigid_S) != len(route.S).")
    ...
    def _build_record_targets(self, batch: Dict[str, Any]) -> list[Dict[str, Any]]:
        ...
        if mode != "source_image_refs":
            raise ValueError("Stage5_2 requires record_views=source_image_refs.")
