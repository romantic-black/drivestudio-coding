...
def masked_rgb_loss(
    ...
    *,
    ...
    mse = torch.mean((pred.detach() - gt.detach()) ** 2).clamp_min(1.0e-12)
...
def delta_regularization(delta: DeltaPack, *, weight: float) -> tuple[torch.Tensor, Dict[str, float]]:
    ...
    out = float(weight) * reg
    return out, {"loss_delta_reg": float(out.detach().item()), "delta_l2": float(reg.detach().item())}
