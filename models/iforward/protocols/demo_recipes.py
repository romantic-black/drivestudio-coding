...
def iforward_demo_cfg(cfg: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "default_recipe": str(_cfg_get(raw, "default_recipe", "repair_showcase_24") or "repair_showcase_24"),
        ...
        "memory_ablation": [
            ...
            for x in list(
                _cfg_get(
                    raw,
                    "memory_ablation",
                    ["full", "memory_off", "memory_read_write", "memory_freeze_write", "memory_shuffle_state"],
                )
                or []
            )
        ],
...
def build_demo_v0_plans(
    *,
    ...
    if recipe_name in {"repair_showcase_24", "distributional_episode_showcase", "repair_tail_showcase", "shuffle_vs_chronological_showcase"}:
    ...
    if recipe_name in {"memory_ablation_showcase", "memory_ablation_distribution_showcase"}:
        ...
        plans = tuple(
            adapter.plan_from_episode_v3(
                episode,
                f"{recipe_name}/{mode}/scene{int(scene_id)}/segment{int(segment_id)}",
                memory_mode=mode,
                source="demo_recipe",
        ...
        return DemoRecipeResult(recipe=recipe_name, plans=plans)
...
def _repair_showcase_plan(
    *,
    ...
    episode: Any,
    protocol_name: str,
    recipe_name: str = "repair_showcase_24",
) -> EpisodePlan:
