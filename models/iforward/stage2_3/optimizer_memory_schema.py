...
class DenseDeltaKVOptimizerState:
    ...
    def detach(self) -> "DenseDeltaKVOptimizerState":
        return DenseDeltaKVOptimizerState(
            kv_state=self.kv_state.detach().clone(),
            seen=self.seen.detach().clone(),
            update_count=self.update_count.detach().clone(),
            last_visit_step=self.last_visit_step.detach().clone(),
            last_frame_id=self.last_frame_id.detach().clone(),
    ...
    def to(self, *, device: torch.device, dtype: Optional[torch.dtype] = None) -> "DenseDeltaKVOptimizerState":
        return DenseDeltaKVOptimizerState(
            kv_state=self.kv_state.to(device=device, dtype=dtype or self.kv_state.dtype),
            seen=self.seen.to(device=device),
            update_count=self.update_count.to(device=device),
            last_visit_step=self.last_visit_step.to(device=device),
            last_frame_id=self.last_frame_id.to(device=device),
...
class KeyedDeltaKVOptimizerState:
    ...
    def detach(self) -> "KeyedDeltaKVOptimizerState":
    ...
    def to(self, *, device: torch.device, dtype: Optional[torch.dtype] = None) -> "KeyedDeltaKVOptimizerState":
        return KeyedDeltaKVOptimizerState(
            keys=self.keys.to(device=device),
            kv_state=self.kv_state.to(device=device, dtype=dtype or self.kv_state.dtype),
            seen=self.seen.to(device=device),
...
class DeltaKVOptimizerBranchState:
    ...
    def detach(self) -> "DeltaKVOptimizerBranchState":
        return DeltaKVOptimizerBranchState(
            dense=None if self.dense is None else self.dense.detach(),
            keyed=None if self.keyed is None else self.keyed.detach(),
        )
...
class ParentOptimizerDeltaKVState:
    ...
    @classmethod
    def empty(cls) -> "ParentOptimizerDeltaKVState":
        return cls(
            bg=DeltaKVOptimizerBranchState(),
            distant=DeltaKVOptimizerBranchState(),
            rigid=DeltaKVOptimizerBranchState(),
    ...
    def detach(self) -> "ParentOptimizerDeltaKVState":
    ...
    def count_tokens(self) -> Dict[str, float]:
        out: Dict[str, float] = {
            "parent_optimizer_gdkv_global_update_step": float(self.global_update_step),
            "parent_optimizer_mamba_global_update_step": float(self.global_update_step),
        }
        for branch in ("bg", "distant", "rigid"):
            for key, value in getattr(self, branch).count().items():
                out[f"parent_optimizer_gdkv_{branch}_{key}"] = float(value)
                out[f"parent_optimizer_mamba_{branch}_{key}"] = float(value)
        return out
