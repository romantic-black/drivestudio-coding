...
class IForwardState:
    ...
    def detach_for_next_rollout(self) -> "IForwardState":
        return IForwardState(
            ...
            sequence10_bank=(
                None
                if self.sequence10_bank is None
                else self.sequence10_bank.detach()
                if hasattr(self.sequence10_bank, "detach")
                else self.sequence10_bank
            ),
            stage2_2_bank=(
                None
                if self.stage2_2_bank is None
