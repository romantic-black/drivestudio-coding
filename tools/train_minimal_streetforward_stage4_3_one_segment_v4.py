"""
Minimal StreetForward Stage 4.3 — 单段训练：TrainSchedulerV4 + MultiSceneDatasetV3（image-ref batch）。

默认配置：
  conda run -n drivestudio-new env PYTHONPATH=/root/drivestudio-coding \
    python tools/train_minimal_streetforward_stage4_3_one_segment_v4.py \
      --config_file configs/minimal_streetforward_stage4_3_one_segment_v4.yaml
"""
...
from tools.train_minimal_streetforward_stage4_1_one_segment_v3 import (
    _build_multi_scene_dataset_v3,
    _build_scheduler_node_sync,
    _build_train_scheduler_v4,
...
CKPT_PREFIX = "minimal_sf_stage4_3_one_segment_v4"
...
def main() -> None:
    ...
    parser.add_argument(
        "--config_file",
        type=str,
        default="configs/minimal_streetforward_stage4_3_one_segment_v4.yaml",
    ...
    logger.info(
        "Building MultiSceneDatasetV3; training one segment scene_id=%s segment_id=%s include_test=%s",
        scene_id,
        segment_id,
        include_test,
    )
    dataset = _build_multi_scene_dataset_v3(cfg, device)
    dataset.initialize()
    scheduler = _build_train_scheduler_v4(cfg, dataset, scene_id, segment_id, include_test)
        ...
        for step in range(max_iterations):
            ...
            for ev in step_events:
                if ev.get("type") == "segment_begin":
                    logger.info(
                        "SEGMENT_BEGIN epoch=%s global_step=%s scene=%s segment=%s U=%s segment_budget_u=%s segment_step_budget=%s updates_per_block=%s",
                        ev.get("epoch_idx"),
                        ev.get("global_step"),
                        ev.get("scene_id"),
                        ev.get("segment_id"),
                    ...
                    logger.info(
                        "RESET global_step=%s scene=%s segment=%s reset_episode_idx=%s reason=%s window=%s num_pairs=%s",
                        ev.get("global_step"),
                        ev.get("scene_id"),
                        ev.get("segment_id"),
                    ...
                    logger.info(
                        "BLOCK_BEGIN global_step=%s scene=%s segment=%s block_seg=%s block_global=%s U=%s K_u_nom=%s K_u_eff=%s K_steps_eff=%s source_kf=%s source_frame=%s source_image_ref=%s",
                        ev.get("global_step"),
                        ev.get("scene_id"),
                        ev.get("segment_id"),
            ...
            if step % image_interval == (image_interval - 1) or step == 0:
