"""
Training script for Minimal StreetForward Stage 2.0 (multi-target, no proxy).

- Same base as Stage 1.1; uses multiple targets (e.g. 3), loss = mean(loss_i), single backward.
- Logs per-view and mean PSNR/SSIM/LPIPS; saves per-view pred/gt/error images.

Use with overfit batch (with multiple targets):
  python tools/train_minimal_streetforward_stage2_0.py --config_file configs/minimal_streetforward_stage2_0.yaml \\
...
CKPT_PREFIX = "minimal_sf_stage2_0"


def main():
    parser = argparse.ArgumentParser(
        description="Train Minimal StreetForward Stage 2.0 (multi-target, no proxy)"
    )
    parser.add_argument(
        "--config_file",
        type=str,
        default="configs/minimal_streetforward_stage2_0.yaml",
    ...
    logger.info("Building MinimalStreetForwardStage2_0...")
    model = MinimalStreetForwardStage2_0(config=cfg, device=device)
    model.train()
