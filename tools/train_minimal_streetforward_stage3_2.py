"""
Training script for Minimal StreetForward Stage 3.2 (Stage 3.1 + SSIM loss).

Same logging / metrics / test-at-end structure as Stage 3.1.

Use with overfit batch:
  python tools/train_minimal_streetforward_stage3_2.py --config_file configs/minimal_streetforward_stage3_2.yaml \\
...
CKPT_PREFIX = "minimal_sf_stage3_2"


def main():
    parser = argparse.ArgumentParser(
        description="Train Minimal StreetForward Stage 3.2 (2D + bg + distant + sky + SSIM loss)"
    )
    parser.add_argument(
        "--config_file",
        type=str,
        default="configs/minimal_streetforward_stage3_2.yaml",
    ...
    logger.info("Building MinimalStreetForwardStage3_2...")
    model = MinimalStreetForwardStage3_2(config=cfg, device=device)
    model.train()
