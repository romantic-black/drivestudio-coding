"""
Training script for Minimal StreetForward Stage 1 (NodeState + single target).

- Same pipeline as Stage 0 script: overfit one batch, same config/eval/logging.
- Model: MinimalStreetForwardStage1 (NodeStateBackground + NodeState + offset head).

Use with overfit batch:
  python tools/train_minimal_streetforward_stage1.py --config_file configs/minimal_streetforward_stage1.yaml \\
...
def convert_batch_to_minimal_format(batch: Dict, device: torch.device) -> Dict:
    """Convert raw overfit/dataset batch to minimal format (same as Stage 0 script)."""
...
def main():
    parser = argparse.ArgumentParser(description="Train Minimal StreetForward Stage 1 (overfit one batch)")
    parser.add_argument(
        "--config_file",
        type=str,
        default="configs/minimal_streetforward_stage1.yaml",
    ...
    logger.info("Building MinimalStreetForwardStage1...")
    model = MinimalStreetForwardStage1(config=cfg, device=device)
    model.train()
    ...
    try:
        ...
        for step in range(max_iterations):
            ...
            if save_every and step > 0 and step % save_every == 0:
                ckpt_path = os.path.join(cfg.log_dir, "checkpoints", f"minimal_sf_stage1_step{step}.pt")
                torch.save(
                    {
                        "step": step,
    ...
    final_ckpt = os.path.join(cfg.log_dir, "checkpoints", "minimal_sf_stage1_final.pt")
    torch.save({
