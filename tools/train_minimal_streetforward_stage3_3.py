"""
Training script for Minimal StreetForward Stage 3.3.

Keeps Stage 3.2 training/metrics/logging pipeline, while switching trainer/config.
"""
...
CKPT_PREFIX = "minimal_sf_stage3_3"


def main():
    parser = argparse.ArgumentParser(description="Train Minimal StreetForward Stage 3.3")
    parser.add_argument(
        "--config_file",
        type=str,
        default="configs/minimal_streetforward_stage3_3.yaml",
    ...
    logger.info("Building MinimalStreetForwardStage3_3...")
    model = MinimalStreetForwardStage3_3(config=cfg, device=device)
    model.train()
                    ...
                    if test_viewdirs_list is not None and idx < len(test_viewdirs_list):
                        ...
                        if vd.shape[0] != h or vd.shape[1] != w:
                            vd = torch.nn.functional.interpolate(
                                vd.permute(2, 0, 1).unsqueeze(0), size=(h, w), mode="bilinear", align_corners=False
